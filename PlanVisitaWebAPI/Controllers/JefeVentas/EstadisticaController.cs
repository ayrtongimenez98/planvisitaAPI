...
namespace PlanVisitaWebAPI.Controllers.JefeVentas
{
    ...
    public class EstadisticaController : ApiController
    {
        ...
        // GET: api/Estadistica
        public HttpResponseMessage Get([FromUri] int[] vendedorIds, DateTime fechaDesde, DateTime fechaHasta)
        {
            var response = Request.CreateResponse();

            var listaVendedores = db.Vendedor.Where(x => vendedorIds.Contains(x.Vendedor_Id)).ToList();
...
and vc.Sucursal_Id = h.Address where vc.Vendedor_Id = " + item.Vendedor_Id).ToList<SucursalVendedorResponseModel>();
                ...
                estadistica.Porcentaje = asignados.Count > 0 ? Convert.ToInt32((realizado.Count * 100) / asignados.Count) : 0;
